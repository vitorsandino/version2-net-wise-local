...
const DNSQueryStats = ({ serverId }: DNSQueryStatsProps) => {
  ...
  const fetchTopDomains = async () => {
    ...
    const { data, error } = await supabase
      .from("dns_query_logs")
      .select("domain, query_type, query_count, client_ip")
    ...
    const mapClients: Record<string, number> = {};
    ...
    (data || []).forEach(row => {
      ...
      const type = (row.query_type || 'A').toUpperCase();
      ...
      if (clientIp) {
        mapClients[clientIp] = (mapClients[clientIp] || 0) + count;
      }
    ...
    const sortTopClients = (map: Record<string, number>): ClientStats[] => {
      return Object.entries(map)
    ...
    setTopClients(sortTopClients(mapClients));
      ...
      <div className="space-y-2 max-h-[300px] overflow-y-auto">
        {topClients.map((item, index) => (
          <div 
            ...
            className="flex items-center gap-2 p-2 rounded-lg hover:bg-muted/20 transition-colors"
          >
            <span className="w-5 h-5 flex items-center justify-center rounded-full bg-blue-500/20 text-blue-500 text-xs font-bold shrink-0">
              {index + 1}
            </span>
            <div className="flex-1 min-w-0">
              <span className="font-mono text-sm truncate block">{item.client_ip}</span>
              <div className="flex items-center gap-2 mt-0.5">
                <div className="h-1 bg-blue-500/20 rounded-full flex-1 max-w-[100px]">
                  <div 
                    className="h-full bg-blue-500 rounded-full"
                    style={{ width: `${Math.min(100, (item.count / (topClients[0]?.count || 1)) * 100)}%` }}
                  />
                </div>
                <span className="text-xs text-muted-foreground shrink-0">
                  {getPercentage(item.count, totalQueries)}%
                </span>
              </div>
            </div>
            <span className="font-bold text-sm shrink-0">{formatNumber(item.count)}</span>
      ...
      <CardContent>
        {loading ? (
          ...
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p className="text-sm mt-1">O coletor irá enviar dados a cada 60s</p>
            <p className="text-xs mt-2 opacity-70">Verifique: journalctl -u dns-query-collector -f</p>
          </div>
        ) : (
          <>
            {/* Stats Summary */}
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-4">
              ...
              <div className="flex items-center gap-2 p-2 rounded-lg bg-purple-500/10">
                ...
                <div>
                  ...
                  <p className="font-bold text-purple-500">{topClients.length}</p>
                </div>
              </div>
            </div>
            ...
            <Tabs defaultValue="ipv4" className="w-full">
              ...
              <TabsContent value="ipv4" className="mt-0">
                <div className="text-sm text-muted-foreground mb-2">
                  Top 10 domínios resolvidos via IPv4 ({statsA.count} únicos)
                </div>
              ...
              <TabsContent value="ipv6" className="mt-0">
                <div className="text-sm text-muted-foreground mb-2">
                  Top 10 domínios resolvidos via IPv6 ({statsAAAA.count} únicos)
                </div>
              ...
              <TabsContent value="failed" className="mt-0">
                <div className="text-sm text-muted-foreground mb-2">
                  Top 10 domínios com falha de resolução
                </div>
              ...
              <TabsContent value="clients" className="mt-0">
                <div className="text-sm text-muted-foreground mb-2">
                  Top 10 IPs que mais consultam o DNS
                </div>
                {renderClientList()}
              </TabsContent>
              ...
              <TabsContent value="other" className="mt-0">
                <div className="text-sm text-muted-foreground mb-2">
                  Outros tipos (MX, TXT, CNAME, PTR, etc.)
                </div>
